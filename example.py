...
csv_file = Path("/workspaces/Adsorbase/adsorbents.csv")
df2 = pd.read_csv(csv_file, sep= ",")
df2.head()



fig_pca = px.scatter(df2, # data to pull from
...
app_pca = molplotly.add_molecules(fig=fig_pca,  # figure to populate
                                  ...
                                  caption_cols=['Conditions P', 'Conditions T'], # columns to take the additional captions from
...
app_pca.run(jupyter_mode="tab", port=8404)